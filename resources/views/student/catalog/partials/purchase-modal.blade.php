        ...
        <div class="p-6">
            ...
            <div class="space-y-3 mb-6">
                ...
                <div id="modal-discount-row" class="hidden flex justify-between items-center text-green-600">
                    <span>Знижка на курс:</span>
                    <span id="modal-discount-amount" class="font-semibold"></span>
                </div>
                <div id="modal-individual-discount-row" class="hidden flex justify-between items-center text-brand-600">
                    <span>Персональна знижка:</span>
                    <span id="modal-individual-discount" class="font-semibold"></span>
                </div>
                <div id="modal-final-price-row" class="hidden flex justify-between items-center border-t pt-3">
                    <span class="text-gray-900 font-semibold">До оплати:</span>
                    <span id="modal-final-price-display" class="text-xl font-bold text-gray-900"></span>
                </div>
            </div>
            ...
            <form id="purchase-form" method="POST" action="">
                ...
                <div class="flex gap-3">
                    ...
                    >
                        <span id="purchase-btn-text">Оплатити <span id="modal-final-price"></span></span>
                        <span id="purchase-btn-loading" class="hidden items-center justify-center">
...
function openPurchaseModal(courseId, name, instructor, duration, price, discount, imageUrl, individualDiscount = null, finalPrice = null) {
    ...
    const individualDiscountRow = document.getElementById('modal-individual-discount-row');
    if (individualDiscount) {
        document.getElementById('modal-individual-discount').textContent = '-' + individualDiscount;
        individualDiscountRow.classList.remove('hidden');
    } else {
        individualDiscountRow.classList.add('hidden');
    }

    const finalPriceRow = document.getElementById('modal-final-price-row');
    const displayPrice = finalPrice || price;
    document.getElementById('modal-final-price').textContent = displayPrice;

    if (discount || individualDiscount) {
        document.getElementById('modal-final-price-display').textContent = displayPrice;
