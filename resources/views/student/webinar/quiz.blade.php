...
@section('title', $quiz->title . ' - ' . $webinar->title)
...
<div class="px-4 sm:px-6 py-4">
    <div class="flex items-center justify-between">
        <nav class="flex items-center gap-2 text-sm text-gray-600">
            <a href="{{ route('student.dashboard') }}" class="hover:text-teal-600 transition-colors">Дашборд</a>
            <svg class="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
            </svg>
            <a href="{{ route('student.webinar.show', $webinar) }}" class="hover:text-teal-600 transition-colors truncate max-w-40">{{ $webinar->title }}</a>
            <svg class="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
            </svg>
            <span class="text-gray-900 font-medium truncate max-w-48">Тестування</span>
        </nav>
    </div>
</div>

<div class="flex flex-col lg:flex-row" x-data="{
    showQuizForm: {{ $hasPassed ? 'false' : 'true' }},
    quizStarted: true
}">
    <div class="flex-1 lg:pr-0">
        <div class="px-4 sm:px-6 lg:px-8 py-6 max-w-4xl">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-900 mb-2">{{ $quiz->title ?? 'Тестування' }}</h2>
                <div class="flex flex-wrap items-center gap-2 text-sm text-gray-600">
                    <span>Вебінар: {{ $webinar->title }}</span>
                </div>
            </div>
        </div>
            ...
            <div class="px-4 sm:px-6 lg:px-8 py-6 max-w-4xl" x-show="!showQuizForm">
                <div class="p-4 bg-green-50 border border-green-200 rounded-lg">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        @if($canAttempt)
                            <button type="button" @click="showQuizForm = true" class="inline-flex items-center px-4 py-2 text-sm font-medium text-teal-700 bg-teal-100 rounded-lg hover:bg-teal-200 transition">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="font-medium text-yellow-800">
                                Ви вичерпали всі спроби для проходження тесту
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div x-data="quizWizard({{ json_encode($questionsData) }})"
                ...
                @if($maxAttempts !== null)
                    <div class="mb-4 text-sm text-gray-600">
                        Залишилось спроб: <span class="font-semibold">{{ $maxAttempts - $attemptsUsed }} з {{ $maxAttempts }}</span>
                    </div>
                @endif
                        ...
                        <div class="bg-gray-50 rounded-lg p-6">
                            ...
                            @switch($question->type->value)
                                ...
                                @case('drag_drop')
                                    ...
                                    @break
                                @case('ordering')
                                    @include('student.quiz.partials.question-ordering', ['question' => $question, 'questionIndex' => $index])
                                    @break
                            @endswitch
                        </div>
                    </div>
                ...
                <div x-show="isReviewStep" x-cloak class="mb-6">
                    ...
                    <div class="space-y-3">
                        <template x-for="(q, i) in questions" :key="q.id">
                            <div @click="goToQuestion(i)" class="p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors border border-gray-200">
                ...
                <div class="flex items-center justify-between gap-4 mt-6">
                    <button type="button" x-show="currentStep > 0" @click="previous()" class="inline-flex items-center px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                    ...
                    <div class="flex gap-3">
                        <button type="button" x-show="!isReviewStep && currentStep < totalQuestions - 1" @click="next()" class="inline-flex items-center px-6 py-2.5 text-sm font-medium text-white bg-teal-500 rounded-lg hover:bg-teal-600 transition">
                            Далі
                            <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                        </button>

                        <button type="button" x-show="!isReviewStep && currentStep === totalQuestions - 1" @click="goToReview()" class="inline-flex items-center px-6 py-2.5 text-sm font-medium text-white bg-teal-500 rounded-lg hover:bg-teal-600 transition">
                            Переглянути відповіді
                            <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                        </button>

                        <button type="button" x-show="isReviewStep" @click="submitQuiz()" class="inline-flex items-center px-8 py-2.5 text-sm font-medium text-white bg-teal-500 rounded-lg hover:bg-teal-600 transition">
                            Завершити тест
                ...
                <form x-ref="quizForm" action="{{ route('student.webinar.quiz.submit', $webinar) }}" method="POST" class="hidden">
                    @csrf
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-6 max-w-4xl border-t border-gray-200">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                @if($hasPassed)
                    <button type="button" disabled class="inline-flex items-center justify-center px-6 py-2.5 text-sm font-medium text-white bg-green-500 rounded-lg cursor-default">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        Пройдено
                    </button>
                @endif
                <a href="{{ route('student.webinar.show', $webinar) }}" class="inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
