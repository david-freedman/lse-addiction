...
<div class="rounded-2xl border border-gray-200 bg-white p-6">
    <form id="course-edit-form" action="{{ route('admin.courses.update', $course) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        ...
        <div x-data="{ activeTab: '{{ $hasCertErrors ? 'certificate' : 'main' }}' }">
            ...
            <div x-show="activeTab === 'main'" class="space-y-5">
                <div x-data="{ copied: false }">
                    <label class="mb-2 block text-sm font-medium text-gray-700">Зовнішній ID курсу</label>
                    <div class="relative">
                        <input
                            type="text"
                            value="c{{ $course->id }}"
                            readonly
                            @click="navigator.clipboard.writeText('c{{ $course->id }}'); copied = true; setTimeout(() => copied = false, 2000)"
                ...
                <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <input
                            ...
                            value="1"
                            {{ old('requires_certificate_approval', $course->requires_certificate_approval) ? 'checked' : '' }}
                ...
                <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <input
                            ...
                            id="sync_to_wp"
                            value="1"
                            {{ old('sync_to_wp', $course->sync_to_wp) ? 'checked' : '' }}
                            class="mt-1 rounded border-gray-300 text-brand-500 focus:ring-brand-500"
                        >
                        <div>
            ...
            <div x-show="activeTab === 'certificate'" class="space-y-5">
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('cert_company_name', $course->cert_company_name ?? 'Медична академія') }}"
                ...
                <div>
                    ...
                    <input
                        ...
                        name="cert_bpr_hours"
                        id="cert_bpr_hours"
                        value="{{ old('cert_bpr_hours', $course->cert_bpr_hours) }}"
                        min="1"
                    ...
                    >
                    <p class="mt-1 text-xs text-gray-500">Вводиться вручну, незалежно від тривалості курсу</p>
                ...
                <div>
                    ...
                    <input
                        ...
                        id="cert_specialties"
                        value="{{ old('cert_specialties', $course->cert_specialties) }}"
                ...
                <div>
                    ...
                    >
                        <option value="">Не вказано</option>
                        <option value="student" {{ old('cert_participant_type', $course->cert_participant_type) === 'student' ? 'selected' : '' }}>Студент</option>
                        <option value="trainer" {{ old('cert_participant_type', $course->cert_participant_type) === 'trainer' ? 'selected' : '' }}>Тренер</option>
                    </select>
                ...
                <div>
                    ...
                    @if($course->cert_signature)
                        <div class="mb-3">
                            <img src="{{ \Storage::disk('public')->url($course->cert_signature) }}" alt="Поточний підпис" class="max-h-24 rounded-lg border border-gray-200 bg-gray-50 p-2">
                        </div>
                    @endif
                    <input
                        type="file"
                ...
                <div>
                    ...
                    @if($course->cert_stamp)
                        <div class="mb-3">
                            <img src="{{ \Storage::disk('public')->url($course->cert_stamp) }}" alt="Поточна печатка" class="max-h-24 rounded-lg border border-gray-200 bg-gray-50 p-2">
                        </div>
                    @endif
                    <input
                        type="file"
