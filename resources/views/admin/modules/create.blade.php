...
<div class="mb-6">
    ...
    <h1 class="text-title-xl font-bold text-gray-900 mt-1">Створити модуль</h1>
                ...
                <div>
                    ...
                    >
                        @foreach($moduleUnlockRules as $rule)
                            <option value="{{ $rule->value }}" {{ old('unlock_rule', 'none') === $rule->value ? 'selected' : '' }}>
                                {{ $rule->label() }}
                            </option>
                        @endforeach
                    </select>
