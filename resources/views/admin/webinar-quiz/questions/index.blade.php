...
@section('title', 'Питання тесту вебінару')

@section('content')
<div class="mb-6">
    <a href="{{ route('admin.webinars.edit', $viewModel->webinar()) }}" class="text-sm text-gray-500 hover:text-gray-700">
        ← Назад до вебінару
    </a>
    <h1 class="text-title-xl font-bold text-gray-900 mt-1">Питання тесту</h1>
    <p class="text-gray-500">{{ $viewModel->webinar()->title }}</p>
</div>
...
<div class="mb-6 flex items-center justify-between">
    ...
    <a href="{{ route('admin.webinar-quiz.questions.create', $viewModel->webinar()) }}"
...
@if($viewModel->questions()->isEmpty())
    <div class="rounded-2xl border border-gray-200 bg-white p-12 text-center">
        ...
        <p class="mt-2 text-gray-500">Додайте перше питання до тесту</p>
        <a href="{{ route('admin.webinar-quiz.questions.create', $viewModel->webinar()) }}"
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-4 hover:shadow-sm transition">
                <div class="flex items-start justify-between">
                    <div class="flex items-start gap-4">
                        <div class="flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 text-sm font-medium text-gray-600">
                            {{ $loop->iteration }}
                        </div>
                        <div>
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin.webinar-quiz.questions.edit', [$viewModel->webinar(), $question]) }}"
                           class="rounded-lg border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
                            Редагувати
                        </a>
                        <form action="{{ route('admin.webinar-quiz.questions.destroy', [$viewModel->webinar(), $question]) }}"
                              method="POST"
